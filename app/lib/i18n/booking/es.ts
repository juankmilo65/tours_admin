/**
 * Spanish Translations - Bookings Component
 */

export const bookingEs = {
  sectionTitle: 'Gestión de Reservas',
  allBookings: 'Todas las Reservas',
  newBooking: 'Nueva Reserva',
  editBooking: 'Editar Reserva',
  deleteBooking: 'Eliminar Reserva',
  bookingDetails: 'Detalles de la Reserva',
  confirmationCode: 'Código de Confirmación',
  status: 'Estado',
  allStatus: 'Todos los Estados',
  bookingDate: 'Fecha de Reserva',
  tourDateRange: 'Rango de Fechas del Tour',
  startDate: 'Fecha de Inicio',
  endDate: 'Fecha de Fin',
  tourDate: 'Fecha Tour',
  tourStart: 'Inicio',
  tourEnd: 'Fin',
  tourInfo: 'Información del Tour',
  numberOfPeople: 'Número de Personas',
  totalPrice: 'Precio Total',
  currency: 'Moneda',
  tour: 'Tour',
  customer: 'Cliente',
  email: 'Correo',
  phone: 'Teléfono',
  country: 'País',
  payments: 'Pagos',
  history: 'Historial',
  actions: 'Acciones',
  viewDetails: 'Ver Detalles',
  cancelBooking: 'Cancelar Reserva',
  noBookingsFound: 'No se encontraron reservas',
  noBookingsDescription: 'Intenta ajustar tus filtros o agrega una nueva reserva',
  showing: 'Mostrando',
  to: 'a',
  of: 'de',
  results: 'resultados',
  previous: 'Anterior',
  next: 'Siguiente',
  pending: 'Pendiente',
  partial: 'Parcial',
  paid: 'Pagado',
  cancelled: 'Cancelado',
  urgent: 'Urgente',
  requested: 'Solicitada',
  confirmed: 'Confirmada',
  pending_confirmation: 'Pendiente de confirmación',
  pendingPayment: 'Pago pendiente',
  partiallyPaid: 'Pago parcial',
  pending_payment: 'Pago pendiente',
  partially_paid: 'Pago parcial',
  notSpecified: 'No especificado',
  cancellationReason: 'Motivo de Cancelación',
  confirmCancellation: 'Confirmar Cancelación',
  keepBooking: 'Mantener Reserva',
  confirmCancellationButton: 'Confirmar Cancelación',
  created: 'Creado',
  lastUpdated: 'Última Actualización',
  cancelledAt: 'Cancelado En',
  lastReminderSent: 'Último Recordatorio Enviado',
  noPayments: 'Sin pagos',
  paymentMethod: 'Método de Pago',
  transactionId: 'ID de Transacción',
  transactionReference: 'Referencia',
  notes: 'Notas',
  paidAt: 'Pagado En',
  totalBookings: 'Total de Reservas',
  pendingBookings: 'Reservas Pendientes',
  paidBookings: 'Reservas Pagadas',
  urgentBookings: 'Reservas Urgentes',
  allUsers: 'Todos los Usuarios',
  allCities: 'Todas las Ciudades',
  tours: {
    selectTour: 'Seleccionar tour',
    tour: 'Tour',
    tourRequired: 'El tour es requerido',
  },
  clients: 'Clientes',
  companions: 'Acompañantes',
  viewCompanions: 'Ver acompañantes',
  addClient: 'Agregar Cliente',
  clientName: 'Nombre del Cliente',
  clientAge: 'Edad del Cliente',
  clientNamePlaceholder: 'Ingrese nombre del cliente',
  clientAgePlaceholder: 'Edad',
  clientsRequired: 'Al menos un cliente es requerido',
  nationality: 'Nacionalidad',
  selectNationality: 'Seleccionar nacionalidad',
  idType: 'Tipo de ID',
  selectIdType: 'Seleccionar tipo de ID',
  clientId: 'Número de ID',
  enterClientId: 'Ingrese el número de ID',
  selectCurrency: 'Seleccionar moneda',
  createSuccess: 'Reserva creada exitosamente',
  createError: 'Error al crear la reserva',
  creatingBooking: 'Creando reserva...',
  creating: 'Creando reserva...',
  bookingForMe: '¿Esta reserva es para ti?',
  isPrimary: 'Principal',
  setPrimary: 'Marcar como principal',
  noPrimaryClient: 'Debes marcar un cliente como principal para continuar',
  // Validation messages
  endDateAfterStartDate: 'La fecha de fin debe ser posterior a la fecha de inicio',
  clientNameMinLength: 'El nombre debe tener al menos 3 caracteres',
  clientNameMaxLength: 'El nombre no puede exceder 100 caracteres',
  clientAgeInteger: 'La edad debe ser un número entero',
  clientAgeMin: 'La edad no puede ser negativa',
  clientAgeMax: 'La edad no puede ser mayor a 120 años',
  clientAgeUnder18: 'Los menores de 18 años deben ser acompañados por un adulto',
  clientIdMinLength: 'El ID debe tener al menos 3 caracteres',
  clientIdMaxLength: 'El ID no puede exceder 50 caracteres',
  hasSpecialRequests: 'Agregar solicitudes especiales',
  specialRequestsPlaceholder: 'Ej. restricciones dietéticas, idioma preferido...',
  specialRequestsRequired: 'Por favor ingresa tus solicitudes especiales o desmarca la casilla',
  specialRequestsMinLength: 'Las solicitudes especiales deben tener al menos 10 caracteres',
  specialRequestsMaxLength: 'Las solicitudes especiales no pueden exceder 500 caracteres',
  validationErrorsTitle: 'Por favor corrige los siguientes errores:',
  loadIdTypesError: 'Error al cargar los tipos de identificación',
  tourItinerary: 'Itinerario del Tour',
  viewItinerary: 'Ver itinerario',
  loadingTourInfo: 'Cargando información del tour...',
  dayLabel: 'Día',
  editClient: 'Editar Cliente',
  primaryLabel: 'Principal',
  yearsOld: 'años',
  select: 'Seleccione',
  selectUser: 'Seleccionar Usuario',
  useSystemUser: 'Usar usuario del sistema',
  noUserSelected: 'Debes seleccionar un usuario',
  clientIdLabel: 'ID Cliente',
  enterEmail: 'Ingresar correo',
  emailLabel: 'Correo',
  nationalityLabel: 'Nacionalidad',
  idTypeLabel: 'Tipo de ID',
  cancel: 'Cancelar',
  save: 'Guardar',
  add: 'Agregar',
  priceSummary: 'Resumen de Precios',
  basePricePerPerson: 'Precio base por persona',
  subtotal: 'Subtotal',
  iva: 'IVA (16%)',
  minorDiscount: 'Descuento menores (10%)',
  minimumPayment: 'Pago mínimo',
  resendPaymentLink: 'Reenviar enlace de pago',
  paymentLinkSentTitle: 'Enlace de pago enviado',
  paymentLinkSentMessage: 'El enlace de pago fue reenviado exitosamente.',
  paymentLinkSentError: 'Error al reenviar el enlace de pago',
  // Multi-tour strings
  multiTourBooking: 'Reserva multi-tour',
  removeAllToursToDisable: 'remueve todos los tours para desactivar',
  selectedTours: 'Tours Seleccionados',
  addToPackage: 'Agregar al Paquete',
  addAnotherTourButton: 'Agregar Otro Tour',
  tourAlreadyAdded: 'Este tour ya está agregado para esta fecha.',
  removeFromBundleButton: 'Remover de Paquete',
  reorderToursMessage: 'Arrastra para reordenar tours en el mismo día',
  sameDayWarning: 'Horarios muy ajustados entre tours: {minutes} minutos de margen',
  sameDayMarginCritical: '⚠️ Menos de 1 hora entre tours - revisa bien los horarios',
  tourNotAvailable: 'Tour no disponible en esta fecha',
  multiTourValidationError: 'Error validando tours',
  multiTourMinRequired: 'Debes agregar al menos un tour al paquete',
  selectTourButton: 'Seleccionar Tour',
  noToursAdded: 'No hay tours agregados',
  addTourToList: 'Agregar Tour a la Lista',
  cancelAddTour: 'Cancelar',
  // Timeline
  depositPaidEvent: 'Depósito Pagado',
  finalPaymentPaidEvent: 'Pago Final',
  transferredToOwnerEvent: 'Transferido al Propietario',
  refundInitiatedEvent: 'Reembolso Iniciado',
  refundCompletedEvent: 'Reembolso Completado',
  // Cash payment
  cashPayment: {
    button: 'Finalizar pago en efectivo',
    title: 'Finalizar pago en efectivo',
    balanceLabel: 'Saldo a cobrar en efectivo',
    balanceHint: 'Monto pre-IVA a cobrar en mano',
    confirm: 'Confirmar pago',
    cancel: 'Cancelar',
    success: 'Pago en efectivo registrado',
  },
  balancePayment: {
    button: 'Pagar saldo online',
    title: 'Pagar saldo con tarjeta / OXXO',
    chooseMethod: 'Elegí el método de pago',
    methodLocal: 'Tarjeta nacional',
    methodForeign: 'Tarjeta internacional',
    methodOxxo: 'OXXO',
    pay: 'Ir a pagar',
    cancel: 'Cancelar',
  },
  // Stripe fee disclosure (informational only — estimated amounts, no payment changes)
  stripeFee: {
    anticipoLabel: 'Anticipo (pago online)',
    feeHeader: 'Comisión de Stripe (la cubre el cliente)',
    localLabel: 'Tarjeta local',
    foreignLabel: 'Tarjeta extranjera',
    oxxoLabel: 'OXXO',
    excedenteLabel: 'Excedente',
    meetingPointNote: 'Pago en efectivo en el punto de encuentro, sin comisión',
    conversionNote: '+2% si hay conversión de moneda',
    inclFee: 'incl. comisión',
    selectMethodHint: 'Elegí el método de pago para ver la comisión estimada.',
    balanceHowLabel: '¿Cómo se paga el excedente?',
    balanceCashOption: 'Efectivo',
    balanceCardOption: 'Tarjeta',
    cardTypeLabel: 'Tipo de tarjeta',
    cardLocalOption: 'Nacional',
    cardForeignOption: 'Extranjera',
    neto: 'Neto',
    comision: 'Comisión',
    totalToPay: 'Total a pagar',
  },
};
